...
def detect_image(model, image, img_size=416, conf_thres=0.5, nms_thres=0.5):
    ...
    with torch.no_grad():
        ...
        #t2 = time()
        #print("Toatal inference time {}".format(t2-t1))
...
def draw_output_image(image, detections, classes):
    """Draws detections in output image and stores this.
...
"""
@timer_func
def simple_detect(model, img):
    rt = detect_image(model = model,image=img,img_size=128)
    rt = torch.tensor(rt)
    _draw_and_save_output_image(img_path, rt, 416, "output", classes)
    print(rt)
"""
def simple_detect(model, img):
    rt = detect_image(model = model,image=img,img_size=128)
